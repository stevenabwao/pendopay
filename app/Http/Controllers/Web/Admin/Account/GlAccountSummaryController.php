...
use App\Services\GlAccountSummary\GlAccountSummaryIndex;
use App\User;
use Carbon\Carbon;
use Excel;
...
class GlAccountSummaryController extends Controller
{
    ...
    /**
     * GlAccountSummaryController constructor.
     *
     * @param GlAccountSummary $model
     */
    public function __construct(GlAccountSummary $model)
    {
        $this->model = $model;
     ...
     */
    public function index(Request $request, GlAccountSummaryIndex $glAccountSummaryIndex)
    {

        //get the data
        $companies = getAllUserCompanies($request);

        $data = $glAccountSummaryIndex->getData($request);
        ...
        return view('accounts.gl-accounts.gl-accounts-summary.index', [
            'glaccountssummary' => $data->appends(Input::except('page')),
            'companies' => $companies
        ]);
    ...
    public function edit($id)
    {
        ...
        if ($user->hasRole('superadministrator')){
            ...
            return view('accounts.gl-accounts.savings.edit', compact('account', 'companies', 'products', 'statuses'));
    ...
    public function show($id)
    {
        ...
        $glaccountsummary = $this->model->find($id);

        return view('accounts.gl-accounts.gl-accounts-summary.show', compact('glaccountsummary'));
