...
use App\Entities\LoanApplicationDecline;
use App\Services\LoanApplication\LoanApplicationCheck;
use Carbon\Carbon;
use Illuminate\Support\Facades\DB;
use Dingo\Api\Exception\StoreResourceFailedException;

class LoanApplicationStoreTest
{
    ...
    public function createItem($attributes) {
        ...
        if (!$deposit_account) {
            ...
            $message = "An error occured. Please contact " . $company_data->name . " admin";
            //dd($message);
            //log_this($message);
        ...
        $attributes['interest_cycle'] = $loan_product_setting->interest_cycle;
        ...
        //dump("here", $company_user_id, $company_product_id);
        try {
            $loan_exposure_limit_data = getLoanExposureLimitData($company_user_id, $company_product_id);
            //dd($loan_exposure_limit_data);
        } catch(\Exception $e) {
            //DB::rollback();
            //dd($e);
            $message = "Error processing loan. Please try again later.";
            log_this($message . "<br> No loan_exposure_limit_data record found.");
        ...
        if ($loan_exposure_limit_data) {
            ...
            //start check if user has set max_loan_limit in loan_exposure_limits table
            $user_max_loan_limit = $loan_exposure_limit_data->max_limit;
        } else {
            //use company default exposure limit
            $user_loan_exposure_limit = $initial_exposure_limit;
            //default value
            $user_max_loan_limit = 0;
        }
        ...
        try {
            ...
            //dd($e);
            $message = "Error processing loan right now. Please try again later.";
            log_this($message . "<br> No user_reg_payments record found - " . $e->getMessage());
            throw new StoreResourceFailedException($message);
        }
        ...
        try {
            ...
            //dd($e);
            $message = "Error processing loan right now. Please try again later.";
            log_this($message . "<br> No user_shares_payments record found - " . $e->getMessage());
            throw new StoreResourceFailedException($message);
        }
        ...
        try {
            $user_set_loan_limit = getUserSetLoanLimit($loan_limit_calculation_id, $user_deposit_payments,
                                                   $user_reg_payments, $user_shares_payments, 
                                                   $initial_loan_limit);
        } catch(\Exception $e) {
            //dd($e);
            $message = "Error processing loan right now. Please try again later.";
            log_this($message . "<br> No user_set_loan_limit record found - " . $e->getMessage());
            throw new StoreResourceFailedException($message);
        }
        ...
        $attributes['user_max_limit_at_application'] = $user_max_loan_limit;
