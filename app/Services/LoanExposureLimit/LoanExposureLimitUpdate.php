...
use App\Services\LoanExposureLimit\LoanExposureLimitApprove;
use Carbon\Carbon;
        ...
        DB::beginTransaction();
            ...
            if ($trans_status == 'completed') {
                ...
                try {
                    ...
                    $the_loan_application = LoanExposureLimit::find($request_id);
                ...
                try {
                    ...
                    $loan_application_approve = new LoanExposureLimitApprove(); 
                    ...
                    if ($result_json->status_code == 200) {
                        ...
                        //start save loan accounts report section data
                        //report section items
                        $report_section_loan_accounts = config('constants.report_section.loan_accounts');
                        $section_name = config('constants.report_section.active_loan_accounts');
                        $ref_txt = "Successful loan application";
                        $current_date_obj = getCurrentDateObj();
                        $data_parent_id = $request_id;
                        ...
                        try {
                            ...
                            $result = createNewReportSectionData($report_section_loan_accounts, $company_id,
                                $loan_amount, $current_date_obj, $data_parent_id, $payment_id,
                        ...
                        //end save repayments report section data

                        
                        if (isEmailOk($email, $company_id)) {

                            //get loan account id
                            $loan_account_data = LoanAccount::where('loan_application_id', $request_id)->first();
                            $loan_account_id = $loan_account_data->id;

                            //update loan account count
                            updateCompanyLoanAccount($loan_account_id, $section_name, $company_id);
                    ...
                    if ($loans_exceeding_limit == "queue") {
                        ...
                        //store audits
                        storeLoanExposureLimitAudit($request_id);
