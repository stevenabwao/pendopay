...
class ManageDataIndexGd 
{
	...
	public function getData($request)
	{
        ...
        //PROCESS DATA
        //1. PROCESS ALMOST EXPIRING LOAN REPAYMENTS
        

        //2. PROCESS NON-REGISTERED MEMBERS

        //3. PROCESS EXPIRED LOAN REPAYMENTS - REMINDERS
        ...
        DB::beginTransaction();
            ...
            //loan account update sections
            $completed_loan_accounts = config('constants.report_section.completed_loan_accounts');
            $overdue_loan_accounts = config('constants.report_section.overdue_loan_accounts');
                ...
                if ($target_loan_accounts) {
                    ...
                    foreach ($target_loan_accounts as $target_loan_account) {
                        ...
                        if ($reminder_active) {
                            ...
                            /*$email_reminder = ReminderMessage::where('loan_account_id', $loan_account_id)->first();
            ...
            //***********************************************************//
            //5. UPDATE LOAN ACCOUNT DATA - START
            //***********************************************************//

            //processUpdateOverdueLoanAccounts();

            processUpdateOverdueLoanAccountsCount();

            //***********************************************************//
            //5. UPDATE LOAN ACCOUNT DATA - END
            //***********************************************************//
