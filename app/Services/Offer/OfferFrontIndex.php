...
class OfferFrontIndex
{
	...
	public function getData($request)
	{
        ...
        if ($expiry_date) {
            ...
            // get utc date
            $expiry_date_utc = formatUTCDate($expiry_date);
        ...
        if ($start_date) {
            $data = $data->where('offers.created_at', '>=', $start_date_utc);
        }
        if ($end_date) {
            $data = $data->where('offers.created_at', '<=', $end_date_utc);
        }
        if ($expiry_date) {
            $data = $data->where('offers.expiry_at', $expiry_date_utc);
        }

        // get offers with at least one configured product
        if ($min_products) {
            $data = $data->where('offers.num_products', '>=', $min_products);
        }
