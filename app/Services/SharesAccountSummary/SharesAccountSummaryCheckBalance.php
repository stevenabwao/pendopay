...
class SharesAccountSummaryCheckBalance
{
	...
	public function getData($attributes)
	{
        ...
        $section_name = "SharesBalanceInquiry";
        ...
        $ledger_balance_data = DB::table('accounts')
            ->join('shares_accounts', 'accounts.id', '=', 'shares_accounts.ref_account_id')
            ->join('shares_account_summary', 'shares_accounts.account_no', '=', 'shares_account_summary.account_no')
            ->select('shares_account_summary.ledger_balance')
        ...
        $message = "$short_name, \n\n";
        $message .= " Account Name: $account_name,\n Shares Balance: $ledger_balance_fmt";
