...
class SupervisorClientAssignStore {
    ...
    public function createItem($attributes) {
        ...
        DB::beginTransaction();
            ...
            if (array_key_exists('usersSelected', $attributes)) {
                $usersSelected = $attributes['usersSelected'];
            }

            //get client ids
            $client_ids_array = explode(",", $usersSelected);
            //dd($client_ids_array);
            ...
            //dd($supervisor_data, $company_supervisor_id);

            //start create main supervisor client assign record
            try {
                ...
                if ($client_data) {
                    ...
                    try {
                        ...
                        if (!$client_assigned_data) {
                            ...
                            //client is not assigned yet, proceed
                            try {

                                $new_supervisor_client_assign = new SupervisorClientAssignDetail();
                                $assign_attributes['supervisor_client_assign_id'] = $supervisor_client_assign_id;
                                $assign_attributes['company_user_id'] = $client_company_user_id;
                                $assign_attributes['company_supervisor_id'] = $supervisor_id;
                                $assign_attributes['company_id'] = $client_company_id;
                                $assign_attributes['created_by'] = $auth_user_id;
                                $assign_attributes['created_by_name'] = $auth_user_full_names;
                                $assign_attributes['updated_by'] = $auth_user_id;
                                $assign_attributes['updated_by_name'] = $auth_user_full_names;
                                ...
                                $supervisor_client_assign_result = $new_supervisor_client_assign->create($assign_attributes);

                                log_this(">>>>>>>>> SNB SUCCESS ASSIGNING USER TO SUPERVISOR :\n\n" . json_encode($supervisor_client_assign_result) . "\n\n\n");
                                

                                //update company user assign
                                updateCompanyUserAssign($client_company_user_id, $client_company_id, $supervisor_id);
                                ...
                                //dd($e);
                                $message = $e->getMessage();
                                log_this(">>>>>>>>> SNB ERROR ASSIGNING USER TO SUPERVISOR :\n\n" . $message . "\n\n\n");
                                $response["message"] = $message;
                                return show_json_error($response);
                            ...
                            DB::rollback();
                            $message = "Client " . $client_full_names . " is already assigned to supervisor: " . $assigned_supervisor_full_names;
                            log_this(">>>>>>>>> SNB ERROR ASSIGNING USER - ALREADY ASSIGNED :\n\n" . $message . "\n\n\n");
                            $response["message"] = $message;
                            return show_json_error($response);
                        ...
                        //dd($e);
                        $message = $e->getMessage();
                        log_this(">>>>>>>>> SNB ERROR ASSIGNING CLIENT TO SUPERVISOR :\n\n" . $message . "\n\n\n");
                        $response["message"] = $message;
                        return show_json_error($response);
                    ...
                    log_this(">>>>>>>>> SNB ERROR ASSIGNING USER NOT IN SAME COMPANY AS SUPERVISOR :\n\n" . $message . "\n\n\n");
                    $response["message"] = $message;
                    return show_json_error($response);
                }
